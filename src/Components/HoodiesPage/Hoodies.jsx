...
import "../HoodiesPage/Hoodies.css";
import Footer from "../Footerpart/Footer";
import { Link } from "react-router-dom";
import { useDispatch, useSelector } from "react-redux";
import { fetchHoodiesCollection } from "../../toolkit/slice";

const Hoodies = () => {
  const dispatch = useDispatch();
  const hoodieCollectionData = useSelector(
    (state) => state.data.hoodieCollectionData
  );

  useEffect(() => {
    dispatch(fetchHoodiesCollection());
  }, [dispatch]);

  return (
    <section className="container hoodie-page">
      <section className="top-part">
        <div className="pt-3 d-flex justify-content-between">
          <ArrowBackIosNewIcon />
          <span className="fw-bold">Hoodies</span>
      ...
      <section className="row pt-4">
        {hoodieCollectionData
          ? hoodieCollectionData.map((item) => (
